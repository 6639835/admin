      ...
      {data.length === 0 ? (
        ...
        <div className="space-y-4">
          ...
          {data.map((post, index) => (
            <div
              key={post.post_slug}
              className="p-4 bg-gray-50 dark:bg-slate-800/30 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-700/50 transition-all group"
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  <span className="flex-shrink-0 w-8 h-8 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-full flex items-center justify-center font-bold text-sm shadow-sm">
                    {index + 1}
                  </span>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-3 text-sm">
                <div className="text-secondary">
                  ...
                  <span className="text-primary">{post.views.toLocaleString()}</span>
                </div>
                <div className="text-secondary">
